...
var saveCookie = function(reURL, title, model, imageURL) {
	if (reURL == "") {
		alert("The product details to jump path (reURL) cannot be empty");
		return;
	}
	if (title == "") {
		alert("Product title (the title) cannot be empty");
		return;
	}
	if (model == "") {
...
var saveNewsCookie = function(reURL, title) {
	if (reURL == "") {alert("Press the jump path (reURL) cannot be empty");return;}
	if (title == "") {alert("News title (the title) cannot be empty");return;}
	var arr2 = new Array();
	 ...
	 try{
	       ...
	       if(window.netscape){
	          try{
	              ...
	              alert("Hello, your browser does not support automatic Settings page for the home page, please manually set the page to the home page in the browser!");
	          }
	       }else{
	        alert("Sorry, the browser you are using nn was unable to complete this operation you need to manually set the href "+ +" to the home page");
	       }
	  }
}
...
var check_reg = function(){
	...
	if(!validator.required($("#registerDiv #registerPassword").val())){
		errorStr+="Password cant be empty！";
		flag=false;
	}
	if(!validator.required($("#registerDiv #confirmPassword").val())){
		errorStr+="Confirm password cannot be empty！";
		flag=false;
	...
	if(!validator.required($("#registerDiv #emailName").val())){
		errorStr+=lan['product.enterEmail'];
		flag=false;
	}else if(!validator.isEmail($("#registerDiv #emailName").val())){
		errorStr = lan['dolog.confirmEmail'];
			flag=false;
	}
	if(!flag){
		alert(errorStr);
...
$(function() {
	...
	/*$.each($("#registerDiv .aui_se"),function(){
		scale($(this).get(0), 480, 220, function(x, y) {
			if (x > 480) {
				$("#registerDiv .aui_content").width(x);
			}
			if (y > 220) {
				$("#registerDiv .aui_content").height(y);
			}
		});
	});*/
	...
	$("#loginSubmit").live("click",function() {
		...
		$.ajax({
			type : "get",
			url : "http://en.jihui88.com/rest/api/shop/member/login",
			async : false,
			dataType : "jsonp",
...
$(function() {
    ...
    function left() {
        if (parseInt($(window).width()) - parseInt($(container).width()) < 20) {
            return container.offsetLeft + container.offsetWidth - 20
        } else {
